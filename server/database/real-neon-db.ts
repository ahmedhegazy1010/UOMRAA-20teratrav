...
async function insertRealData() {
  try {
    ...
    // حذف جميع الباقات وإعادة إنشائها
    ...
    // التحقق من وجود أي باقات
    const packageCount = await sql`SELECT COUNT(*) as count FROM packages`;
    console.log("Package count before insert:", packageCount[0].count);

    console.log(
      "Condition check: packageCount[0].count === 0?",
      packageCount[0].count === 0,
    );
    console.log(
      "Condition type check: count =",
      packageCount[0].count,
      "type =",
      typeof packageCount[0].count,
    );

    if (Number(packageCount[0].count) === 0) {
      console.log("Inserting real Umrah packages...");
      ...
      await sql`
        ...
        VALUES 
          ...
          ('رجب المبارك (فبراير)', '5 أيام / 4 ليالي', '3 ليالي - فندق دار التوحيد', '1 ليلة - فندق الطيبة سيتي', 'الرياض - المدينة - مكة - جدة - الرياض', 3200, 2800, 2400, 1200, 2000, 'active', false, 'باقة قصيرة لشهر رجب المبارك مناسبة للرحلات السريعة'),
                    ('شعبان المبارك (مارس)', '6 أيام / 5 ليالي', '3 ليالي - فندق سويس أوتيل', '2 ليالي - فندق شذا المدينة', 'الرياض - جدة - مكة - المدينة - جدة - الرياض', 3800, 3300, 2900, 1400, 2500, 'active', false, 'باقة شهر شعبان مع خدمات متميزة وإقامة مريحة')
      `;
      console.log("✅ Real Umrah packages created");
    } else {
      console.log(
        "Skipping package insertion - packages already exist or count check failed",
      );
    }
  } catch (error) {
